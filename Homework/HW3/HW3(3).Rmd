...
```
  ...
  $P(\text{pick 6 first} \cap \text{pick 17 second}) = {1/400} \times {1/400} = 6.25 \times 10^{-6}$
...
```
  ...
  $n\{ A \} = \sum_{1}^{18} = \frac{18 \times 19}{2} = 171$
...
```
  ...
  d) Now assume the first number drawn is not replaced back into the box.  Repeat (a) - (c).
    ...
    d.a)
    $$ P(6 \cap 17 \text(w/o replacement)) = \frac{1}{n} \times \frac{1}{n-1} =  0.002$$
```{r}
(1/20)*(1/19)
```
