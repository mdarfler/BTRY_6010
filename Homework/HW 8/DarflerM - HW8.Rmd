...
>Given a significance level of 0.05, i.e. 5% chance of a type-I error, and a p-value of `r round(results$p.value, 3)` we would reject the null hypothesis in favor of the alternative hypothesis $\mu_d \neq 0$.
...
> At the 5% signifcance level there is evidence that the mean score for students with elevated pulses is different from the mean score for students without elevated pulses. This is because the lower bound of the 95% confidence interval is > 0. 
...
> Yes, the estimate for the significance level of the test does indicated that it was 0.05. Specifically, the results of our simulation showed that `r (1 - mean(rej)) * 100`% of the time the means were the statistically the same. 
