...
$$\begin{align*} P(X = 10 | X \leq 8) &= \dfrac{P(X = 10) \cap P(X \geq 8)}{P(X \geq 8)}\\
&=\dfrac{P(X = 10)}{P(X \geq 8)}\\
&=\dfrac{\binom{10}{10}0.8^{10}(1-0.8)^0}{\sum_{i=0}^{8}\binom{10}{i}0.8^i(1-0.8)^{10-i}}\\
&=\dfrac{\texttt{dbinom(10,10.0.8)}}{\texttt{pbinom(8,10,0.8)}}\\
&= \dfrac{0.107}{0.6242} = 0.172 \end{align*}$$
```{r}
...
$X = \{ (1,1), (1,2), (2,1)\}  \quad \quad Y = \{ (4,6), (5,5), (6,5), (6,4), (6,5), (6,6)\}$

$n\{ X \} < n\{Y\}$

 >This game is not fair because there are only three ways for Harold to win and there are six ways for Timmy to win
 
 b) If Timmy wins in round 1, what is his expected earnings?
 
 >E(X) = $1 \times \frac{9}{12}$ (Chance of picking a pip card) + $5 \times \frac{3}{12}$ (chance of picking a face card) $= 2$
...
P(X) &= (0.0069 + 0.0278) \times 0.0625 = 0.0022
...
P(X = -1) &= \frac{6}{36} \times \frac{9}{12} = 0.125\\
...
EXT = EX1 + EX2
paste0("The total expected number of surviving butterflies is ",EXT)
```
...
>I'd say that the calculated value from the Poisson distribution (0.0029599) is a pretty good approximation for this Binomial Distribution (0.0029575). 
