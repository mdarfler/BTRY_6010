...
>Yes, because we know that fuel economy is normally distributed and we know the population standard deviation $\sigma$ 
...
$$ 95\% ci = (\bar x - SE * z_{\alpha/2}, \bar x + SE * z_{\alpha/2}) $$
```{r}
48.3 - SE * qnt
48.3 + SE * qnt
```
...
> yes, because $n\hat p_n \geq 10 \text{ and } n(1-\hat p_n) \geq 10$ 
...
```
  ...
  $$SE(\hat p_n)$ = $\sqrt{\frac{\hat p_n(1-\hat p_n)}{n}}$$
...
```
  ...
  > This confidence interval is larger than the ci calculated from above where the population standard deviation is known
