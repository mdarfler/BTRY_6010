...
$$X \sim Binomilla(n,p) \quad \text{with known n and unknown p}$$
  ...
  $$X \sim Binomial(n, 0.25)$$
  
  2) We would like to test whether Dr. Basu is psychic.  This implies that the actual probability of Dr. Basu guessing the correct number for a given student is greater than what we would expect by random guessing.  This establishes the **alternative hypothesis** of our test, $H_a: p > 0.25$ Dr. Basu *is* psychic.
  ...
  $$P(X > x_{rejection}) = \alpha = 0.05$$
```{r}
for(i in 8:12){
print(paste0(i, ": ",pbinom(i, 30, 0.25, lower.tail = F)))
}
```
...
(reject_lower <- qnorm(alpha/2, mu, SE))
(reject_upper <- qnorm(1-alpha/2, mu, SE))

(1-pnorm(1.7,mu,SE))*2
```
>$x_{reject} = 1.5 \pm 2.58 \times \frac{\sigma}{\sqrt n} = (1.34, 1.66)$ Reject $H_0 \quad if \quad 1.34 < \bar X < 1.66$.
