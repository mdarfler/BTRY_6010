...
$$SD (2x) = \sqrt{Var(2X)} = \sqrt{4 Var(x)} = 2 \times SD(x)$$
...
$$SD(X + 4) = \sqrt{Var(X+4)} = \sqrt{Var(X)} = SD(X)$$
...
The proportion of Norwegians in their early 40s who drink at least 1 cup of coffee per day is about p = 0.894.  Suppose that we take a random sample of 50 Norwegians from this age group.  Let X = the number of Norwegians in the sample that drink at least 1 cup of coffee a day.  In your answers to the questions below, include the correct notation for the probability being asked for in terms of the random variable X.
  ...
  $$X \sim Binomial(50, 0.894) \\
  E[X] = 50 \times 0.894 = 44.7$$
...
```
  ...
  $$sd = \sqrt{np(1-p)} = \sqrt{50 \times 0.894 \times (1 - 0.894)} = \sqrt{4.7382} = 2.177$$
```{r}
50 * 0.894 * 0.106
sqrt(.Last.value)
```
  ...
  $$\begin{align*}
  P(X = k) &= \binom{n}{k} p^k(1-p)^{n-k} \\
  P(X = 48) &= \binom{50}{48} 0.894^{48}(1-0.894)^{50-48} \\
  &= \dfrac{50!}{48!(50 - 48)!} 0.894^{48} (0.106) ^2 \\
  &= 0.0635
  
  \end{align*}$$
```{r}
choose(50,48) * 0.894^48 * 0.106^2
```
 ...
 $$ \begin{align*}
 P(X ≤ 47) &= \sum_{i = 1}^{47} \binom{50}{i} p^i (1-p)^{50 - i} \\
 &= 1-P(X > 47)
 \end{align*}
$$
```{r}
