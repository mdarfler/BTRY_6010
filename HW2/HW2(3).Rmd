...
```{r}
SS <- read.csv("https://raw.githubusercontent.com/mdarfler/BTRY_6010/master/HW2/StudentSurvey(3).txt")
...
> I would consider this a Non-probability sample because there are many units in the population whose probability of selection is 0, e.g. students NOT in stats. Furthermore, I would consider this a convenience study because it's a lot easier to reach out to the population that you already have access too rather than attempting to randomly sample all university students. This could not be used to make generalized statements about the populations*

  d) List the variable types of the following (be as specific as possible!):
    
        1. TV  - **Numerical, Discrete, Ratio, Integer**
        
        2. Award - **Categorical Factor w/ 3 Levels**
...
t = table(SS$Award)
print(t)
paste0("The most popular award is ", names(which(t == max(t))))
```  
...
> The proportions for each award *were* different for men and women. Women preferred Academy awards more than men, and men preferred Olympic medals more than women. Men and women preferred Nobel prizes at about the same rate.
...
```{r}
boxplot(SS$GPA~SS$Award, xlab = "Award Type", ylab = "GPA", main = "GPA by Award")
```
  
  
  i. Does there appear to be any differences between the median GPAs of the three groups?  Support your answer using information from the plots.
...
>The *outlier* in the dataset belongs to the Nobel group. This is indicated by the circle well below the lower whisker**
...
```{r}
ss.sophomores = subset(SS, Year == "Sophomore")
hist(ss.sophomores$Exercise,
     breaks=20,
...
>The range of exercise is 0 - 40. A student exercising more than half of sample would have to be exercising at least 10 hrs/week since the data is a discrete integer.
...
```{r}
boxplot(SS$Exercise~SS$Award)
```

> Those who aspire for an Olympic medal on average exercise more than those who aspire to a Nobel prize or an academy award.
...
>There seems to be a compression in the variation of scores as TV watching increases but the mean GPA appears to remain the same, i.e. the line of best fit has a slope of 0.
  
  c) Let's take a look at this relationship again after excluding students who watch 40 hours of TV a week or more. Do this in two lines.  First, create a new data frame (using the `subset` function). Second, use `plot`.
  
```{r,eval=TRUE}
# First line: create new data frame that keeps only students with TV under 40  
ss.lessthan40 = subset(SS,TV<40)
# Second line: call plot function with this newly created data frame
plot(ss.lessthan40$TV,ss.lessthan40$GPA)
```
...
ss.females = subset(SS,Gender=="F")
points(ss.females$TV, ss.females$GPA, col='red') # this is third line
```
...
> `points()` draws points at specified coordinates and the argument `col = "red"` colors them red.
...
> It's a pretty terrible image. It's hard to read, or to make observations and inferences from. This is especially true as the number of piercings goes above 7.

  d) What might be a better way to graphically describe the distribution of `Piercings`?
  
> A bar plot might be nice. I would have said a histogram, but since we can no longer considering the values as integers, but rather as factors, the histogram would not be appropriate.

```{r}
barplot(table(SS$Piercings),
        xlab = "# of Piercings",
        ylab = "Count",
        main = "Distribution of Number of Piercings")
```
